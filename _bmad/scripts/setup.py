...
def replace_dir(src: Path, dest: Path) -> None:
    ...
    # mkdtemp() creates the directory; os.rename() onto an existing
    # directory fails on Windows (unlike POSIX), so remove the empty
    # placeholder immediately before renaming into its reserved path.
    backup.rmdir()
    try:
        dest.rename(backup)
    except Exception:
